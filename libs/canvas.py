...
class Canvas(QWidget):
    ...
    def setPolygon(self, actions, filepath, value=True):
        ...
        self.mask_for_grab = np.zeros(self.img_for_grab.shape[:2],dtype = np.uint8) # mask initialized to PR_BG
        self.output = np.zeros(self.img_for_grab.shape,np.uint8)           # output image to be shown
    ...
    def Segment_Roi(self):
        ...
        if self.flag_enter_polygon:     
            ...
            cv2.grabCut(self.img2_for_grab,self.mask_for_grab,self.rect_for_grab,bgdmodel,fgdmodel,1,cv2.GC_INIT_WITH_RECT)
            cv2.grabCut(self.img2_for_grab,self.mask_for_grab,self.rect_for_grab,bgdmodel,fgdmodel,1,cv2.GC_INIT_WITH_MASK)

            mask2 = np.where((self.mask_for_grab==1) + (self.mask_for_grab==3),255,0).astype('uint8')
            self.output = cv2.bitwise_and(self.img2_for_grab,self.img2_for_grab,mask=mask2)
            gray = cv2.cvtColor(self.output,cv2.COLOR_BGR2GRAY)
            ret, binary = cv2.threshold(gray,1,255,cv2.THRESH_BINARY) 
            contours  = cv2.findContours(binary,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE) #,hierarchy
            contours_big_area=[]
            max_contours=0
            max_contours_i=0
            ...
            for i in range(len(contours_big_area[0])):
                self.contours_t.addPoint(QPointF(contours_big_area[0][i][0][0], contours_big_area[0][i][0][1]))
            self.contours_t.addPoint(QPointF(contours_big_area[0][0][0][0], contours_big_area[0][0][0][1]))
    ...
    def canCloseShape(self):
        return self.drawing() and self.ploygon() and self.current and len(self.current) > 2
    ...
    def boundedMoveVertex(self, pos):
        ...
        if len(shape)==4 and ( int(shape[0].x()) == int(shape[3].x()) and int(shape[0].y()) == int(shape[1].y())):
            lindex = (index + 1) % 4
            rindex = (index + 3) % 4
            lshift = None
            rshift = None
    ...
    def finalise(self):
        ...
        if self.actions is not None:
            ...
            self.grab_2_x = xmax
            self.grab_2_y = ymax
            self.rect_for_grab= (self.grab_0_x,self.grab_0_y,int(abs(self.grab_0_x-self.grab_2_x)),int(abs(self.grab_0_y-self.grab_2_y)))
            #bg
            ...
            for i in range(len(self.shapes[len(self.shapes)-1])):
                ...
                cv2.circle(self.mask_for_grab,(self.bg_0_x,self.bg_0_y),self.thickness+2,self.value_for_grab['val'],-1)
        # if self.flag_fg:
